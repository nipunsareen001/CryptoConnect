...
def setLastCheckBlockNumber(blockNumber):
    ...
    cursor.execute('UPDATE LastCheckedTokenABlock SET last_block_number = ? WHERE deposit_type = ? ', (blockNumber, hlp.TokenName.TokenA.value))
    conn.commit()
    hlp.closeDatabase(conn)

def lastCheckBlockNumber():
    cursor, conn = hlp.getDatabase()
    cursor.execute('SELECT last_block_number FROM LastCheckedTokenABlock WHERE deposit_type = ? ', (hlp.TokenName.TokenA.value))
...
def sendExceptionMail(exceptionData):
    subject = 'AUTO-DEPOSIT | TokenA HAS FACED AN EXCEPTION!'
    text_message = (
        "AUTO-DEPOSIT | TokenA HAS FACED AN EXCEPTION!.\r\n"
        "EXception data: .\r\n"
...
def updateExceptionLogs(exceptionData):
    with open("./depositTokenAExceptionLogs.csv", "a", newline="") as csvfile:
...
def updateLogs(logData):
    with open("./depositTokenALogs.csv", "a", newline="") as csvfile:
        writer = csv.writer(csvfile)
...
def handle_event(event):
    try:
        ...
        cursor.execute("EXEC InsertUniqueBlockchainDepositLogWithCheck ?, ?, ?, ?, ?, ?, ?",
            (event['transactionHash'].hex(), event['args']['from'], event['args']['to'], value_token, value_trx_str, hlp.TokenName.TokenA.value, datetime.utcnow()))
